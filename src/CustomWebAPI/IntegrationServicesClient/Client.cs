...
namespace CustomWebAPI.IntegrationServicesClient
{
  class Client
  {
    ...
    public static void SendRequest(string messageRequest) 
    {
      var postRequestToFunction = client.For("DataIntegration").Action("RunScript").Set(new { request = messageRequest }).ExecuteAsync();
    }
  }
}
